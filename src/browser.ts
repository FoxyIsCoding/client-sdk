...
export class VoidAuth {
  private config: Required<VoidAuthConfig>
    ...
    try {
      ...
      if (raw) {
        ...
        if (this.tokens?.idToken) {
          try { this.user = decodeJwtPayload(this.tokens.idToken) } catch {}
        }
      }
    } catch {}
  }
  ...
  async handleCallback(): Promise<{ user: OIDCUser; tokens: OAuthTokens }> {
    ...
    if (saved.state && state !== saved.state) throw new Error('State mismatch')
  ...
  private async fetchUserinfo(accessToken: string): Promise<OIDCUser> {
    const discovery = await fetchDiscovery(this.config.issuer)
